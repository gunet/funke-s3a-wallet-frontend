...
export interface IOpenID4VP {
	handleAuthorizationRequest(url: string, vcEntitylist: ExtendedVcEntity[]): Promise<{ conformantCredentialsMap: Map<string, any>; verifierDomainName: string, verifierPurpose: string, verifierAttestationsJwt: string | null, verifierInfo: Array<{ format: string, data: string, credential_ids?: string[] }> | null, presentationDefinition: any, dcqlQuery: any,  parsedTransactionData: ParsedTransactionData[] | null} | { error: HandleAuthorizationRequestError }>;
	promptForCredentialSelection(conformantCredentialsMap: { [x: string]: number[] }, verifierDomainName: string, verifierPurpose: string, verifierAttestationsJwt: string | null, verifierInfo: Array<{ format: string, data: string, credential_ids?: string[] }> | null, presentationDefinition: any, dcqlQuery: any): Promise<Map<string, number>>;
